...
def makePing(server: MetricsParserModel) -> int:
    return int(
        ping(
            server.servd.ip.__str__(),
            size=server.servd.psz, timeout=server.servd.tmt,
            interval=server.servd.pps, count=server.isa.sea).rtt_avg * 1000)


def executePing(server: MetricsParserModel, api: InstatusAPI) -> None:
    while True:
        pp = InstatusMetricsRequestBodyModel(
            timestamp=int(round(time.time())),
            value=makePing(server))
...
def processPings(servers: List[MetricsParserModel], api: InstatusAPI) -> None:
    for server in servers:
        logging.info(
            f"Starting metrics fetching thread #{servers.index(server)} for server {server.servd.ip}")
